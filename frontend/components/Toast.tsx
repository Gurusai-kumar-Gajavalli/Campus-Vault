    ...
    <ToastContext.Provider value={value}>
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2 pointer-events-none max-w-sm w-full">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border shadow-hover animate-fade-in ${
              t.type === 'success'
                ? 'bg-emerald-50 text-emerald-950 border-emerald-200'
                : t.type === 'error'
                ? 'bg-rose-50 text-rose-950 border-rose-200'
                : 'bg-stone-50 text-ink border-stone-border'
            }`}
          >
            ...
            {t.type === 'info' && <Info className="w-5 h-5 text-moss shrink-0 mt-0.5" />}
